...
- [Integración con TUI](#integración-con-tui-existente)
- [Componentes Bubbletea](#componentes-bubbletea)
- [Comandos de Desarrollo](#comandos-de-desarrollo)
- [Plan de Implementación](#plan-de-implementación-mvp)
...
Juego de entrenamiento de Vim estilo RPG integrado
al TUI installer de Gentleman.Dots. Construido con
**Go + Bubbletea** (Charmbracelet).

**Ubicación**: `installer/`
**Nueva opción de menú**: "🎮 Vim Mastery Trainer"
...
├── internal/
│   ├── system/       # Detección OS, ejecución
│   └── tui/
│       ├── installer.go  # Lógica instalación
│       ├── model.go      # Model principal
│       ├── update.go     # Update handlers
│       ├── view.go       # Views
│       ├── styles.go     # Lipgloss styles
│       ├── keymaps*.go   # Pantallas keymaps
...
Trainer estilo **keybr.com para Vim** con progresión
RPG:
...
| Desbloqueo    | Derrotar jefe desbloquea siguiente   |
| Características | Stats persistentes, spaced repetition |
...
👹 JEFE FINAL (Boss Fight)
    │  Ejercicio épico que combina TODO
...
| `;`      | Repetir f/F/t/T                       |
| `,`      | Repetir f/F/t/T dirección opuesta     |
| `0`      | Inicio de línea                       |
| `$`      | Final de línea                        |
...
**Otros operadores:** Los mismos patterns aplican
para `d` (delete), `y` (yank) y `v` (visual select).
...
| Comando | Descripción                       |
| ------- | --------------------------------- |
| `*`     | Buscar palabra bajo cursor (fwd)  |
| `#`     | Buscar palabra bajo cursor (back) |
| `n`, `N`| Siguiente / anterior match        |
| `gn`    | Seleccionar próximo match         |
...
| Comando               | Descripción               |
| --------------------- | ------------------------- |
| `:s/foo/bar/`         | Línea actual, primera     |
| `:s/foo/bar/g`        | Línea actual, todas       |
| `:%s/foo/bar/g`       | Todo el archivo           |
| `:%s/foo/bar/gc`      | Todo, con confirmación    |
...
| Comando                | Descripción              |
| ---------------------- | ------------------------ |
| `:%s/\s\+$//g`         | Eliminar trailing ws     |
| `:%s/foo/bar/gI`       | Reemplazar exacto        |
...
| Comando              | Descripción                    |
| -------------------- | ------------------------------ |
| `/\vfunction\s+\w+` | Buscar "function" + nombre     |
...
```text
┌─────────────────────────────────────────────┐
│  🎯 TEXT OBJECTS  Nivel 5/10 🔥Racha:7     │
│  ████████████░░░░░░░░ 60%                   │
├─────────────────────────────────────────────┤
│                                             │
│  CÓDIGO:                                    │
...
│  └─────────────────────────────────────────┘│
│                                             │
│  MISIÓN: Cambiá contenido entre comillas    │
│          por "pro" (cursor en 'g' de gentl) │
│                                             │
│                        ⏱️  5.2s             │
├─────────────────────────────────────────────┤
│  Tu input: ci"_                             │
...
│  - c = change (borra y entra en insert)     │
│  - i" = inner quotes (contenido)            │
│                                             │
...
```text
┌─────────────────────────────────────────────┐
│  👹 JEFE: The Nightmare  ❤️ ❤️ ❤️           │
├─────────────────────────────────────────────┤
│                                             │
│  CÓDIGO:                                    │
│  ┌─────────────────────────────────────────┐│
│  │ 1  const█userName = getUser()           ││
...
| ---------------- | ----------------- | ------------------------- |
| Horizontal       | The Line Walker   | Navegar línea sin j/k     |
| Vertical         | The Code Tower    | 50 líneas, llegar a点     |
| Text Objects     | The Bracket Demon | Código anidado, cambiar   |
| Change & Repeat  | The Clone Army    | 10 ocurrencias, cgn       |
| Sustitución      | The Transformer   | Rangos y flags complejos  |
| Regex            | The Pattern Master| Patterns en código real   |
| Macros           | The Automaton     | Macro + múltiples líneas  |

### Mecánicas de Boss

| Mecánica | Descripción                          |
| -------- | ------------------------------------ |
| ❤️ Vidas  | 3 errores y perdés (retry)           |
| ⏱️ Timer  | Más ajustado que práctica normal     |
| Cadena   | 5 pasos seguidos, todo conectado     |
| Combo    | Respuestas rápidas dan bonus (x2-x4) |
| HP       | Barra visual que se reduce           |
...
│   ├── model.go         # Model principal
│   ├── update.go        # Update handlers
│   ├── view.go          # Render pantallas
│   ├── styles.go        # Lipgloss styles
│   ├── exercise.go      # Tipos ejercicios
│   ├── boss.go          # Lógica boss fights
│   ├── stats.go         # Persistencia stats
│   ├── validation.go    # Validar respuestas
│   └── exercises/
│       ├── horizontal.go
│       ├── vertical.go
│       ├── textobjects.go
...
En `model.go`, agregar "🎮 Vim Mastery Trainer"
como opción del menú principal.

### 3. Handler en update.go

Cuando se seleccione la opción del trainer,
cambiar a `ScreenVimTrainer` y delegar al
sub-model del trainer.
...
| Comando                                    | Descripción            |
| ------------------------------------------ | ---------------------- |
| `go build -o gentleman.dots ./cmd/...`     | Build del binario      |
| `go test ./...`                            | Ejecutar todos tests   |
| `./gentleman.dots`                         | Ejecutar installer     |
| `go test ./internal/tui/trainer/... -v`    | Tests del trainer      |
...
| Aspecto  | Guideline                                     |
| -------- | --------------------------------------------- |
| Patterns | Seguir patterns en el TUI                     |
| Estilos  | Usar Lipgloss (`styles.go`)                   |
